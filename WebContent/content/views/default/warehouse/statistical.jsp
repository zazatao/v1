...
<body>
	...
	<div class="form-group">
		<div class="col-md-3">
			<select class="form-control" id="personnelID" >
				<option value="" >选择查询人员
				<c:forEach items="${personnels }" var="personnel">
					<option value="${personnel.id }" <c:if test="${person.id == personnel.id}">selected</c:if>>${personnel.userName }
				</c:forEach>
			</select> <br>
		</div>
	</div>
	<div class="form-group">
		<div class="col-md-1">
			<button class="form-control"  onclick="searchParam();">查询</button>
		</div>
	</div>
...
<script type="text/javascript">
	var PATH = "../analysis/removecauses?param=week&personnelID="+$('#personnelID').val();
	function searchParam(){
		var personnelID = $('#personnelID').val();
		var param = $('#param').val();
		PATH = "../analysis/removecauses?param=" + param+"&personnelID="+personnelID;
		require.config({
			paths : {
