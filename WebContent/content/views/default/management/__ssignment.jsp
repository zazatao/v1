...
<ul>
	<c:forEach var="cur" items="${treeList2}" varStatus="vs">  
	 ...
	 <li><span><i class="icon-minus-sign"></i>${cur.positionname }</span>&nbsp;&nbsp;<input name="positionCheck" value="${cur.positionid }" type="checkbox" class="check">
		  <c:if test="${fn:length(cur.children) > 0}"><!-- 如果有childen -->  
		    ...
		    <c:import url="__ssignment.jsp" /><!-- 这就是递归了 -->  
		  </c:if>
	  </li>   
	</c:forEach>
</ul>  
